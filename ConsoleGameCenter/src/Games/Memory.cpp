	...
	while (!exit)
	{
		...
		while (true)
		{
			...
			if (corrTurns == size * size)
			{
				...
				getEnter("Press ENTER to finish the game...");
				break;
			}
			...
			if (itemsBoard[fc_coords[0]][fc_coords[1]] == itemsBoard[sc_coords[0]][sc_coords[1]])
			{
				...
				getEnter("Press ENTER to remove these cards...");
			}
			else
			{
				Beep(600, 600);
				...
				getEnter("Press ENTER to turn these cards over...");
			}
		}
		...
		std::cout
			...
			<< "Total time spent: " << timeEnd - timeStart << " seconds\n" << std::endl;
